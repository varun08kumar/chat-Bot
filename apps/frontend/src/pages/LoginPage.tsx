  ...
  return (
    <div className="grid h-full place-items-center bg-zinc-50/60 dark:bg-zinc-950">
      <form onSubmit={submit} className="w-full max-w-sm rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <h1 className="mb-1 text-lg font-medium text-zinc-900 dark:text-zinc-50">Log in</h1>
        <p className="mb-5 text-sm text-zinc-500 dark:text-zinc-400">Welcome back.</p>
        ...
        {login.isError && (
          <p className="mb-4 text-sm text-red-500">
            {(login.error as any)?.response?.data?.detail ?? "Something went wrong."}
          </p>
        )}
        ...
        <p className="mt-4 text-center text-sm text-zinc-500 dark:text-zinc-400">
          No account? <Link to="/register" className="text-brand-600 underline dark:text-zinc-200">Sign up</Link>
        </p>
      </form>
    </div>
  );
